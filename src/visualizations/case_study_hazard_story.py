...
print("=" * 80)
print("\n4.2.4 Case Study: Real-Time Hazard Analysis")
print("\nUsing the coefficients from our trained Cox model, we can analyze hazard rate")
print("changes in real professional matches, providing tactical insights into round dynamics.")
print("Figure 10 examines fnatic vs Imperial on Mirage (Round 9, 2024 FiReLEAGUE Global Finals).")
print("\nTimeline Analysis:")
print("\n① 0.00s - Bomb Plant (2v1 Post-Plant)")
print(f"   Hazard Rate: {phase1_hr:.3f}")
print("   Situation: Imperial (T-side) holds 2v1 advantage with bomb planted A Default")
print("   Setup:")
print("     • HEN1: A_Default (Zone 30), AWP, 100 HP, Full Armor")
print("     • decenty: Triple_Conn_Side (Zone 22), AK-47, 100 HP, Full Armor")
print("     • Stairs smoke active (7.5s remaining from pre-plant execute)")
print(f"   Model factors: Zone HR ({decenty_zone_hr:.2f}) × Numerical advantage ({numerical_advantage_hr:.2f}) × Teammates ({num_teammates_hr:.2f}) × Smoke ({has_smoke_hr:.2f})")
print("   Note: Modest HR reduction (2.2%) despite 2v1 advantage - players separated (no crossfire setup)")
print("\n② 0.93s - HEN1 Eliminated")
print(f"   Hazard Rate: {phase2_hr:.3f}")
print("   Event: CT pushes from Ticket Entry, immediately kills HEN1 (141 damage)")
print("   Analysis: Despite CT having no info and many angles to clear, aggressive push succeeds")
print("   New situation: 1v1 - decenty isolated with no teammate support")
print(f"   HR increases {((phase2_hr/phase1_hr - 1)*100):.0f}% due to loss of numerical advantage and crossfire")
print("\n③ 3.50s - decenty Secures Trade")
print(f"   Hazard Rate: {phase3_hr:.3f}")
print("   Event: decenty eliminates CT attacker")
print("   Key: decenty took 0 damage entire round - perfect positioning allowed safe trade")
print("   New situation: 1v0 - only remaining threat is bomb detonation radius")
print(f"   HR drops {((phase3_hr/phase2_hr - 1)*100):.0f}% - no enemy combatants remaining")
print("\n④ 8.48s - Round Complete")
print(f"   Hazard Rate: {phase5_hr:.3f}")
print("   Event: Bomb detonates, decenty clears radius safely")
print("   Result: Imperial wins round")
print("   HR reaches zero - no threats remaining")
print("\nKey Insight: Model-predicted hazard rates align with round outcome - numerical")
print("advantage coefficients accurately captured risk changes as players were eliminated.")
